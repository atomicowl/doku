...
def test_bundled_documentation_workflow_scopes_analysis_to_call_chain():
    ...
    assert 'subagentType: "call-chain-analyzer"' in prompt
    assert 'subagentType: "decision-flow-analyzer"' not in prompt
    assert 'subagentType: "feature-toggle-analyzer"' not in prompt
    assert 'subagentType: "external-dependency-analyzer"' not in prompt
